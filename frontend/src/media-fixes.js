...
window.fetch=async(input,init)=>{
  try{
    ...
    if(raw?.startsWith('https://myinstants-api.vercel.app/search')){
      ...
      const proxy=new URL(`${API}/api/myinstants/search`);
...
document.addEventListener('click',async e=>{
  ...
  if(btn.closest('.gif-grid')){
    const hadTitle=btn.hasAttribute('title'),oldTitle=btn.getAttribute('title');
    btn.setAttribute('title','files');
    setTimeout(()=>{if(hadTitle)btn.setAttribute('title',oldTitle);else btn.removeAttribute('title')},0);
    return;
  }
  ...
  if(btn.closest('.fx-panel')&&btn.dataset.mp3){
    ...
    try{
      await sendMyInstantsSound(btn);
      btn.closest('.fx-panel')?.remove();
