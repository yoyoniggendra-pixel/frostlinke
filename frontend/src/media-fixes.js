...
async function getActiveConversation(){
  ...
  const r=await nativeFetch(`${API}/api/conversations`,{headers:{Authorization:`Bearer ${s?.access_token||''}`}});
...
async function sendMyInstantsSound(btn){
  ...
  const s=await getSession();
  const r=await nativeFetch(`${API}/api/conversations/${c.id}/messages`,{method:'POST',headers:{Authorization:`Bearer ${s?.access_token||''}`,'Content-Type':'application/json'},body:JSON.stringify({type:'sound',body:null,payload:{name:btn.dataset.title||'MyInstants sound',mime:'audio/mpeg',url,myinstants:true}})});
  const j=await r.json().catch(()=>({}));
