...
function installTyping(){
  const input=document.querySelector('.composer input');if(!input||input.dataset.frostTyping)return;
  input.dataset.frostTyping='1';
  const emitStart=()=>{if(!activeConversationId||!realtimeSocket?.connected)return;if(!typingActive){typingActive=true;realtimeSocket.emit('typing:start',{conversation_id:activeConversationId})}clearTimeout(typingTimer);typingTimer=setTimeout(emitStop,900)};
  const emitStop=()=>{clearTimeout(typingTimer);if(typingActive&&realtimeSocket?.connected)realtimeSocket.emit('typing:stop',{conversation_id:activeConversationId});typingActive=false};
...
function installSocket(){
  ...
  token().then(t=>{if(!t)return;realtimeSocket=io(API,{auth:{accessToken:t},transports:['websocket','polling'],reconnection:true});
    realtimeSocket.on('connect',()=>{resolveActive();if(activeConversationId)realtimeSocket.emit('join-conversation',activeConversationId)});
    realtimeSocket.on('typing',x=>{if(x?.conversation_id===activeConversationId&&x.user_id!==realtimeSocket.auth?.userId)setTyping(x.active?`${x.sender||'Someone'} is typing…`:'')});
    realtimeSocket.on('message:read',x=>{if(x?.message_id)addReadStatus(x.message_id,true)});
  }).catch(()=>{});
}
...
function boot(){
  ...
  setInterval(()=>{loadConversations();markIncomingRead()},2500);
}
